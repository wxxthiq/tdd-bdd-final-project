...
class TestProductRoutes(TestCase):
    ...
    def test_get_product_not_found(self):
        """It should not read a missing product"""
        product_id = 0
    ...
    def test_update_product(self):
        ...
        product_data["description"] = "Hello!"
        response = self.client.put(f"{BASE_URL}/{product_data['id']}", json=product_data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        updated_product = response.get_json()
        self.assertEqual(updated_product["description"], "Hello!")

    def test_update_product_not_found(self):
        """It should not Update a not found product"""
    ...
    def test_query_by_availability(self):
        ...
        products = self._create_products(10)
        available = [product for product in products if product.available == True]
        available_count = len(available)
        logging.debug("Found Products [%d] %s", available_count, available)
